  ...
  public:
    ...
    void BeginSession(const std::string& name, const std::string& filepath = "results.json")
    {
      ...
      if (m_CurrentSession)
      {
        ...
        {
          CANDY_CORE_ERROR("Instrumentor::BeginSession('{0}') when session '{1}' already open.", name, m_CurrentSession->Name);
        }
        InternalEndSession();
      }
      ...
      else
      {
        if (Log::GetCoreLogger()) // Edge case: BeginSession() might be before Log::Init()
        {
          CANDY_CORE_ERROR("Instrumentor could not open results file '{0}'.", filepath);
        }
      }
    }
...
#define CANDY_FUNC_SIG __PRETTY_FUNCTION__
  ...
  #define CANDY_PROFILE_BEGIN_SESSION(name, filepath) ::Candy::Instrumentor::Get().BeginSession(name, filepath)
  #define CANDY_PROFILE_END_SESSION() ::Candy::Instrumentor::Get().EndSession()
  #define CANDY_PROFILE_SCOPE_LINE2(name, line) constexpr auto fixedName##line = ::Candy::InstrumentorUtils::CleanupOutputString(name, "__cdecl ");\
											   ::Candy::InstrumentationTimer timer##line(fixedName##line.Data)
