...
#define CANDY_INTERNAL_ASSERT_IMPL_MSG(type, check, msg, ...) { if(!(check)) { CANDY##type##ERROR(msg, __VA_ARGS__); CANDY##type##ERROR("Assertion '{0}' failed at {1}:{2}", CANDY_STRINGIFY_MACRO(check), std::filesystem::path(__FILE__).filename().string(), __LINE__); CANDY_DEBUGBREAK(); } }
#define CANDY_INTERNAL_ASSERT_WITH_MSG(type, check, ...) CANDY_INTERNAL_ASSERT_IMPL_MSG(type, check, "Assertion failed: {0}", __VA_ARGS__)
...
#define CANDY_CORE_ASSERT_MSG(check, msg, ...) { if (!(check)) {CANDY_CORE_ERROR("Assertion '{0}' failed at {1}:{2}", CANDY_STRINGIFY_MACRO(check), std::filesystem::path(__FILE__).filename().string(), __LINE__); CANDY_CORE_ERROR(msg, __VA_ARGS__); CANDY_DEBUGBREAK();}}
