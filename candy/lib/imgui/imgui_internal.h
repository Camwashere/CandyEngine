...
static inline float  ImLengthSqrVec2(const ImVec2& lhs)                             { return (lhs.x * lhs.x) + (lhs.y * lhs.y); }
static inline float  ImLengthSqrVec4(const ImVec4& lhs)                             { return (lhs.x * lhs.x) + (lhs.y * lhs.y) + (lhs.z * lhs.z) + (lhs.w * lhs.w); }
...
struct IMGUI_API ImRect
{
    ...
    //constexpr ImRect(const Engine::Shape::Rectangle& other) : Min(other.min.x, other.max.y), Max(other.max.x, other.min.y){}
    ...
    //operator Engine::Shape::Rectangle()const{return Engine::Shape::Rectangle(Min.x, Max.y, Max.x, Min.y);}
};
...
{
    ImGuiSliderFlags_Vertical               = 1 << 20,  // Should this drag be orientated vertically?
...
struct ImGuiContext
{
    ...
    ImGuiID                 TempInputId;                        // Temporary text input when CTRL+clicking on a drag, etc.
    ...
    float                   SliderCurrentAccum;                 // Accumulated drag delta when using navigation controls.
    bool                    SliderCurrentAccumDirty;            // Has the accumulated drag delta changed since last time we tried to apply it?
