...
// [SECTION] Widgets: ColorEdit4, ColorPicker, ColorButton, etc.
...
bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE* v, float v_speed, const TYPE v_min, const TYPE v_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (is_logarithmic)
    {
        // When using logarithmic sliders, we need to clamp to avoid hitting zero, but our choice of clamp value greatly affects drag precision. We attempt to use the specified precision to estimate a good lower bound.
...
// Convert a value v in the output space of a drag into a parametric position on the drag itself (the logical opposite of ScaleValueFromRatioT)
template<typename TYPE, typename SIGNEDTYPE, typename FLOATTYPE>
float ImGui::ScaleRatioFromValueT(ImGuiDataType data_type, TYPE v, TYPE v_min, TYPE v_max, bool is_logarithmic, float logarithmic_zero_epsilon, float zero_deadzone_halfsize)
{
    ...
    if (is_logarithmic)
    {
        ...
        else if ((v_min < 0.0f) || (v_max < 0.0f)) // Entirely negative drag
...
// Convert a parametric position on a drag into a value v in the output space (the logical opposite of ScaleRatioFromValueT)
template<typename TYPE, typename SIGNEDTYPE, typename FLOATTYPE>
TYPE ImGui::ScaleValueFromRatioT(ImGuiDataType data_type, float t, TYPE v_min, TYPE v_max, bool is_logarithmic, float logarithmic_zero_epsilon, float zero_deadzone_halfsize)
{
    // We special-case the extents because otherwise our logarithmic fudging can lead to "mathematically correct"
    // but non-intuitive behaviors like a fully-left drag not actually reaching the minimum value. Also generally simpler.
    ...
    if (g.ActiveId == id)
    {
        ...
        else if (g.ActiveIdSource == ImGuiInputSource_Keyboard || g.ActiveIdSource == ImGuiInputSource_Gamepad)
        {
            ...
            if (input_delta != 0.0f)
            {
                ...
                if (decimal_precision > 0)
                {
                    input_delta /= 100.0f;    // Gamepad/keyboard tweak speeds in % of drag bounds
                    if (tweak_slow)
                        input_delta /= 10.0f;
                }
                else
            ...
            else if (g.SliderCurrentAccumDirty)
            {
                ...
                else
                {
                    ...
                    // Calculate what our "new" clicked_t will be, and thus how far we actually moved the drag, and subtract this from the accumulator
...
// For 32-bit and larger types, drag bounds are limited to half the natural type range.
...
// Note: p_data, p_min and p_max are _pointers_ to a memory address holding the data. For a drag, they are all required.
...
bool ImGui::VSliderScalar(const char* label, const ImVec2& size, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
    // For the vertical drag we allow centered text to overlap the frame padding
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
...
// Create text input in place of another active widget (e.g. used when doing a CTRL+Click on drag/drag widgets)
...
// ColorEdit4 supports RGB and HSV inputs. In case of RGB input resulting color may have undefined hue and/or saturation.
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    // If we're not showing any drag there's no point in doing any HSV conversions
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        ...
        if (IsItemActive())
        {
            ...
            float initial_dist2 = ImLengthSqrVec2(initial_off);
    ...
    // R,G,B and H,S,V drag color editor
