...
namespace msdfgen {
  ...
  template <>
  class DistancePixelConversion<float> {
    float invRange;
  public:
    typedef BitmapRef<float, 1> BitmapRefType;
    inline explicit DistancePixelConversion(float range) : invRange(1/range) { }
    inline void operator()(float *pixels, float distance) const {
  ...
  void generateDistanceField(const typename DistancePixelConversion<typename ContourCombiner::DistanceType>::BitmapRefType &output, const Shape &shape, const Projection &projection, float range) {
...

    {
      ...
      for (int y = 0; y < output.height; ++y) {
        ...
        for (int col = 0; col < output.width; ++col) {
          ...
          Point2 p = projection.unproject(Point2(x+.5f, y+.5f));
  ...
  void generateSDF(const BitmapRef<float, 1> &output, const Shape &shape, const Projection &projection, float range, const GeneratorConfig &config) {
  ...
  void generatePseudoSDF(const BitmapRef<float, 1> &output, const Shape &shape, const Projection &projection, float range, const GeneratorConfig &config) {
    if (config.overlapSupport)
  ...
  void generateMSDF(const BitmapRef<float, 3> &output, const Shape &shape, const Projection &projection, float range, const MSDFGeneratorConfig &config) {
    if (config.overlapSupport)
  ...
  void generateMTSDF(const BitmapRef<float, 4> &output, const Shape &shape, const Projection &projection, float range, const MSDFGeneratorConfig &config) {
    if (config.overlapSupport)
...
// Legacy API
  ...
  void generateSDF(const BitmapRef<float, 1> &output, const Shape &shape, float range, const msdfVec2 &scale, const msdfVec2 &translate, bool overlapSupport) {
    generateSDF(output, shape, Projection(scale, translate), range, GeneratorConfig(overlapSupport));
  }
  
  void generatePseudoSDF(const BitmapRef<float, 1> &output, const Shape &shape, float range, const msdfVec2 &scale, const msdfVec2 &translate, bool overlapSupport) {
    generatePseudoSDF(output, shape, Projection(scale, translate), range, GeneratorConfig(overlapSupport));
  }
  
  void generateMSDF(const BitmapRef<float, 3> &output, const Shape &shape, float range, const msdfVec2 &scale, const msdfVec2 &translate, const ErrorCorrectionConfig &errorCorrectionConfig, bool overlapSupport) {
    generateMSDF(output, shape, Projection(scale, translate), range, MSDFGeneratorConfig(overlapSupport, errorCorrectionConfig));
  }
  
  void generateMTSDF(const BitmapRef<float, 4> &output, const Shape &shape, float range, const msdfVec2 &scale, const msdfVec2 &translate, const ErrorCorrectionConfig &errorCorrectionConfig, bool overlapSupport) {
...
// Legacy version
  ...
  #pragma omp parallel for
    ...
    for (int y = 0; y < output.height; ++y) {
      ...
      for (int x = 0; x < output.width; ++x) {
        float dummy;
        Point2 p = msdfVec2(x+.5f, y+.5f)/scale-translate;
  ...
  void generatePseudoSDF_legacy(const BitmapRef<float, 1> &output, const Shape &shape, float range, const msdfVec2 &scale, const msdfVec2 &translate) {
    ...
    for (int y = 0; y < output.height; ++y) {
      int row = shape.inverseYAxis ? output.height-y-1 : y;
      for (int x = 0; x < output.width; ++x) {
        Point2 p = msdfVec2(x+.5, y+.5)/scale-translate;
        SignedDistance minDistance;
  ...
  void generateMSDF_legacy(const BitmapRef<float, 3> &output, const Shape &shape, float range, const msdfVec2 &scale, const msdfVec2 &translate, ErrorCorrectionConfig errorCorrectionConfig) {
  ...
  void generateMTSDF_legacy(const BitmapRef<float, 4> &output, const Shape &shape, float range, const msdfVec2 &scale, const msdfVec2 &translate, ErrorCorrectionConfig errorCorrectionConfig) {
