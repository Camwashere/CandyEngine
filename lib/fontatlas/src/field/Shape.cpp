...
namespace msdfgen {
    ...
    for (std::vector<Contour>::iterator contour = contours.begin(); contour != contours.end(); ++contour) {
      if (contour->edges.size() == 1) {
        ...
        for (std::vector<EdgeHolder>::iterator edge = contour->edges.begin(); edge != contour->edges.end(); ++edge) {
          msdfVec2 prevDir = (*prevEdge)->direction(1).normalize();
          msdfVec2 curDir = (*edge)->direction(0).normalize();
  ...
  void Shape::bound(float &l, float &b, float &r, float &t) const {
    for (std::vector<Contour>::const_iterator contour = contours.begin(); contour != contours.end(); ++contour)
      contour->Bound(l, b, r, t);
  }
  
  void Shape::boundMiters(float &l, float &b, float &r, float &t, float border, float miterLimit, int polarity) const {
    for (std::vector<Contour>::const_iterator contour = contours.begin(); contour != contours.end(); ++contour)
      contour->BoundMiters(l, b, r, t, border, miterLimit, polarity);
  }
  
  Shape::Bounds Shape::getBounds(float border, float miterLimit, int polarity) const {
    static const float LARGE_VALUE = 1e240;
  ...
  void Shape::scanline(Scanline &line, float y) const {
  ...
  void Shape::orientContours() {
    ...
    const float ratio = .5*(sqrt(5)-1); // an irrational number to minimize chance of intersecting a corner or other point of interest
    ...
    for (int i = 0; i < (int) contours.size(); ++i) {
      if (!orientations[i] && !contours[i].edges.empty()) {
        ...
        float y0 = contours[i].edges.front()->point(0).y;
        float y1 = y0;
