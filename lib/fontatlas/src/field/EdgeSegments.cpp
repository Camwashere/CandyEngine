...
namespace msdfgen {
  ...
  void EdgeSegment::distanceToPseudoDistance(SignedDistance &distance, Point2 origin, float param) const {
    if (param < 0) {
      msdfVec2 dir = direction(0).normalize();
      msdfVec2 aq = origin-point(0);
      float ts = dotProduct(aq, dir);
      if (ts < 0) {
        float pseudoDistance = crossProduct(aq, dir);
      ...
      msdfVec2 dir = direction(1).normalize();
      msdfVec2 bq = origin-point(1);
      float ts = dotProduct(bq, dir);
      if (ts > 0) {
        float pseudoDistance = crossProduct(bq, dir);
        if (fabs(pseudoDistance) <= fabs(distance.distance)) {
          distance.distance = pseudoDistance;
          distance.dot = 0;
        }
  ...
  Point2 CubicSegment::point(float param) const {
    msdfVec2 p12 = mix(p[1], p[2], param);
    return mix(mix(mix(p[0], p[1], param), p12, param), mix(p12, mix(p[2], p[3], param), param), param);
  }
  
  msdfVec2 LinearSegment::direction(float param) const {
    return p[1]-p[0];
  }
  
  msdfVec2 QuadraticSegment::direction(float param) const {
    msdfVec2 tangent = mix(p[1]-p[0], p[2]-p[1], param);
    if (!tangent)
      return p[2]-p[0];
    return tangent;
  }
  
  msdfVec2 CubicSegment::direction(float param) const {
    msdfVec2 tangent = mix(mix(p[1]-p[0], p[2]-p[1], param), mix(p[2]-p[1], p[3]-p[2], param), param);
    if (!tangent) {
  ...
  msdfVec2 LinearSegment::directionChange(float param) const {
    return msdfVec2();
  }
  
  msdfVec2 QuadraticSegment::directionChange(float param) const {
    return (p[2]-p[1])-(p[1]-p[0]);
  }
  
  msdfVec2 CubicSegment::directionChange(float param) const {
    return mix((p[2]-p[1])-(p[1]-p[0]), (p[3]-p[2])-(p[2]-p[1]), param);
  }
  
  float LinearSegment::length() const {
    return (p[1]-p[0]).length();
  }
  
  float QuadraticSegment::length() const {
    msdfVec2 ab = p[1]-p[0];
    msdfVec2 br = p[2]-p[1]-ab;
  ...
  SignedDistance LinearSegment::signedDistance(Point2 origin, float &param) const {
    msdfVec2 aq = origin-p[0];
    msdfVec2 ab = p[1]-p[0];
    param = dotProduct(aq, ab)/dotProduct(ab, ab);
    msdfVec2 eq = p[param>.5]-origin;
    float endpointDistance = eq.length();
    if (param > 0 && param < 1) {
      float orthoDistance = dotProduct(ab.getOrthonormal(false), aq);
  ...
  SignedDistance QuadraticSegment::signedDistance(Point2 origin, float &param) const {
    ...
    msdfVec2 epDir = direction(0);
    float minDistance = nonZeroSign(crossProduct(epDir, qa))*qa.length(); // distance from A
    param = -dotProduct(qa, epDir)/dotProduct(epDir, epDir);
    {
      epDir = direction(1);
      float distance = (p[2]-origin).length(); // distance from B
  ...
  SignedDistance CubicSegment::signedDistance(Point2 origin, float &param) const {
    ...
    msdfVec2 ab = p[1]-p[0];
    msdfVec2 br = p[2]-p[1]-ab;
    msdfVec2 as = (p[3]-p[2])-(p[2]-p[1])-br;
    ...
    for (int i = 0; i <= MSDFGEN_CUBIC_SEARCH_STARTS; ++i) {
      ...
      msdfVec2 qe = qa+3*t*ab+3*t*t*br+t*t*t*as;
      for (int step = 0; step < MSDFGEN_CUBIC_SEARCH_STEPS; ++step) {
        // Improve t
        msdfVec2 d1 = 3*ab+6*t*br+3*t*t*as;
        msdfVec2 d2 = 6*br+6*t*as;
  ...
  int LinearSegment::scanlineIntersections(float x[3], int dy[3], float y) const {
  ...
  int QuadraticSegment::scanlineIntersections(float x[3], int dy[3], float y) const {
    int total = 0;
  ...
  int CubicSegment::scanlineIntersections(float x[3], int dy[3], float y) const {
    int total = 0;
    int nextDY = y > p[0].y ? 1 : -1;
    x[total] = p[0].x;
    if (p[0].y == y) {
  ...
  void LinearSegment::bound(float &l, float &b, float &r, float &t) const {
    pointBounds(p[0], l, b, r, t);
    pointBounds(p[1], l, b, r, t);
  }
  
  void QuadraticSegment::bound(float &l, float &b, float &r, float &t) const {
    pointBounds(p[0], l, b, r, t);
    pointBounds(p[2], l, b, r, t);
    msdfVec2 bot = (p[1]-p[0])-(p[2]-p[1]);
    if (bot.x) {
  ...
  void CubicSegment::bound(float &l, float &b, float &r, float &t) const {
    pointBounds(p[0], l, b, r, t);
    pointBounds(p[3], l, b, r, t);
    msdfVec2 a0 = p[1]-p[0];
    msdfVec2 a1 = 2*(p[2]-p[1]-a0);
    msdfVec2 a2 = p[3]-3*p[2]+3*p[1]-p[0];
    float params[2];
    int solutions;
  ...
  void CubicSegment::deconverge(int param, float amount) {
    msdfVec2 dir = direction(param);
    msdfVec2 normal = dir.getOrthonormal();
    float h = dotProduct(directionChange(param)-dir, normal);
    switch (param) {
      case 0:
