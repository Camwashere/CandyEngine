...
#define CLASSIFIER_FLAG_ARTIFACT 0x02
  ...
  const float ErrorCorrectionConfig::defaultMinDeviationRatio = 1.11111111111111111f;
  const float ErrorCorrectionConfig::defaultMinImproveRatio = 1.11111111111111111f;

/// The base artifact classifier recognizes artifacts based on the contents of the SDF alone.
  class BaseArtifactClassifier {
  public:
    inline BaseArtifactClassifier(float span, bool protectedFlag) : span(span), protectedFlag(protectedFlag) { }
    /// Evaluates if the median value xm interpolated at xt in the range between am at at and bm at bt indicates an artifact.
    inline int rangeTest(float at, float bt, float xt, float am, float bm, float xm) const {
    ...
    inline bool evaluate(float t, float m, int flags) const {
      return (flags&2) != 0;
    }
  private:
    float span;
...
/// The shape distance checker evaluates the exact shape distance to find additional artifacts at a significant performance cost.
  ...
  public:
    ...
    public:
      inline ArtifactClassifier(ShapeDistanceChecker *parent, const msdfVec2 &direction, float span) : BaseArtifactClassifier(span, parent->protectedFlag), parent(parent), direction(direction) { }
      /// Returns true if the combined results of the tests performed on the median value m interpolated at t indicate an artifact.
      inline bool evaluate(float t, float m, int flags) const {
        if (flags&CLASSIFIER_FLAG_CANDIDATE) {
          ...
          float aWeight = (1-fabs(tVector.x))*(1-fabs(tVector.y));
          ...
          return parent->minImproveRatio*fabsf(newPSD-refPSD) < float(fabsf(oldPSD-refPSD));
        }
        return false;
      }
    private:
    ...
    const float *msd;
    bool protectedFlag;
    inline ShapeDistanceChecker(const BitmapConstRef<float, N> &sdf, const Shape &shape, const Projection &projection, float invRange, float minImproveRatio) : distanceFinder(shape), sdf(sdf), invRange(invRange), minImproveRatio(minImproveRatio) {
      texelSize = projection.unprojectVector(msdfVec2(1));
    }
    inline ArtifactClassifier classifier(const msdfVec2 &direction, float span) {
  ...
  private:
    ...
    float invRange;
    msdfVec2 texelSize;
    float minImproveRatio;
  };
  
  MSDFErrorCorrection::MSDFErrorCorrection() { }
  
  MSDFErrorCorrection::MSDFErrorCorrection(const BitmapRef<byte, 1> &stencil, const Projection &projection, float range) : stencil(stencil), projection(projection) {
    invRange = 1/range;
  ...
  void MSDFErrorCorrection::setMinDeviationRatio(float minDeviationRatio) {
    this->minDeviationRatio = minDeviationRatio;
  }
  
  void MSDFErrorCorrection::setMinImproveRatio(float minImproveRatio) {
...
/// Determines if the channel contributes to an edge between the two texels a, b.
  static bool edgeBetweenTexelsChannel(const float *a, const float *b, int channel) {
    ...
    float t = (a[channel]-.5)/(a[channel]-b[channel]);
    if (t > 0 && t < 1) {
...
/// Marks texel as protected if one of its non-median channels is present in the channel mask.
  ...
  void MSDFErrorCorrection::protectEdges(const BitmapConstRef<float, N> &sdf) {
    ...
    radius = float(PROTECTION_RADIUS_TOLERANCE*projection.unprojectVector(msdfVec2(invRange, 0)).length());
    ...
    radius = float(PROTECTION_RADIUS_TOLERANCE*projection.unprojectVector(msdfVec2(0, invRange)).length());
    ...
    radius = float(PROTECTION_RADIUS_TOLERANCE*projection.unprojectVector(msdfVec2(invRange)).length());
    for (int y = 0; y < sdf.height-1; ++y) {
...
/// Returns the median of the linear interpolation of texels a, b at t.
  static float interpolatedMedian(const float *a, const float *b, float t) {
    return median(
    mix(a[0], b[0], t),
    mix(a[1], b[1], t),
    mix(a[2], b[2], t)
...
/// Returns the median of the bilinear interpolation with the given constant, linear, and quadratic terms at t.
  static float interpolatedMedian(const float *a, const float *l, const float *q, float t) {
    return float(median(
    t*(t*q[0]+l[0])+a[0],
    t*(t*q[1]+l[1])+a[1],
    t*(t*q[2]+l[2])+a[2]
...
/// Checks if a bilinear interpolation artifact will occur at a point where two specific color channels are equal - such points have extreme median values.
  template <class ArtifactClassifier>
  static bool hasDiagonalArtifactInner(const ArtifactClassifier &artifactClassifier, float am, float dm, const float *a, const float *l, const float *q, float dA, float dBC, float dD, float tEx0, float tEx1) {
...
/// Checks if a bilinear interpolation artifact will occur inbetween two diagonally adjacent texels a, d (with b, c forming the other diagonal).
  ...
  void MSDFErrorCorrection::findErrors(const BitmapConstRef<float, N> &sdf) {
    ...
    float hSpan = minDeviationRatio*projection.unprojectVector(msdfVec2(invRange, 0)).length();
    float vSpan = minDeviationRatio*projection.unprojectVector(msdfVec2(0, invRange)).length();
    float dSpan = minDeviationRatio*projection.unprojectVector(msdfVec2(invRange)).length();
    // Inspect all texels.
    for (int y = 0; y < sdf.height; ++y) {
      for (int x = 0; x < sdf.width; ++x) {
      ...
      for (int y = 0; y < sdf.height; ++y) {
        ...
        for (int col = 0; col < sdf.width; ++col) {
          ...
          *stencil(x, row) |= (byte) (ERROR*(
                                            (x > 0 && ((l = sdf(x-1, row)), hasLinearArtifact(shapeDistanceChecker.classifier(msdfVec2(-1, 0), hSpan), cm, c, l))) ||
                                            (row > 0 && ((b = sdf(x, row-1)), hasLinearArtifact(shapeDistanceChecker.classifier(msdfVec2(0, -1), vSpan), cm, c, b))) ||
                                            (x < sdf.width-1 && ((r = sdf(x+1, row)), hasLinearArtifact(shapeDistanceChecker.classifier(msdfVec2(+1, 0), hSpan), cm, c, r))) ||
                                            (row < sdf.height-1 && ((t = sdf(x, row+1)), hasLinearArtifact(shapeDistanceChecker.classifier(msdfVec2(0, +1), vSpan), cm, c, t))) ||
                                            (x > 0 && row > 0 && hasDiagonalArtifact(shapeDistanceChecker.classifier(msdfVec2(-1, -1), dSpan), cm, c, l, b, sdf(x-1, row-1))) ||
                                            (x < sdf.width-1 && row > 0 && hasDiagonalArtifact(shapeDistanceChecker.classifier(msdfVec2(+1, -1), dSpan), cm, c, r, b, sdf(x+1, row-1))) ||
                                            (x > 0 && row < sdf.height-1 && hasDiagonalArtifact(shapeDistanceChecker.classifier(msdfVec2(-1, +1), dSpan), cm, c, l, t, sdf(x-1, row+1))) ||
                                            (x < sdf.width-1 && row < sdf.height-1 && hasDiagonalArtifact(shapeDistanceChecker.classifier(msdfVec2(+1, +1), dSpan), cm, c, r, t, sdf(x+1, row+1)))
          ));
        }
      }
    }
