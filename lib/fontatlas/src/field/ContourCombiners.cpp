...
namespace msdfgen {
  ...
  static void initDistance(float &distance) {
    distance = -DBL_MAX;
  }
  ...
  static float resolveDistance(float distance) {
    return distance;
  }
  
  static float resolveDistance(const MultiDistance &distance) {
  ...
  typename OverlappingContourCombiner<EdgeSelector>::DistanceType OverlappingContourCombiner<EdgeSelector>::distance() const {
    ...
    float innerScalarDistance = resolveDistance(innerDistance);
    float outerScalarDistance = resolveDistance(outerDistance);
