...
namespace msdfgen {
  ...
  static float distVal(float dist, float pxRange, float midValue) {
    if (!pxRange)
      return (float) (dist > midValue);
    return (float) clamp((dist-midValue)*pxRange+.5);
  }
  
  void renderSDF(const BitmapRef<float, 1> &output, const BitmapConstRef<float, 1> &sdf, float pxRange, float midValue) {
    msdfVec2 scale((float) sdf.width/output.width, (float) sdf.height/output.height);
    pxRange *= (float) (output.width+output.height)/(sdf.width+sdf.height);
  ...
  void renderSDF(const BitmapRef<float, 3> &output, const BitmapConstRef<float, 1> &sdf, float pxRange, float midValue) {
    msdfVec2 scale((float) sdf.width/output.width, (float) sdf.height/output.height);
    pxRange *= (float) (output.width+output.height)/(sdf.width+sdf.height);
    for (int y = 0; y < output.height; ++y)
      for (int x = 0; x < output.width; ++x) {
  ...
  void renderSDF(const BitmapRef<float, 1> &output, const BitmapConstRef<float, 3> &sdf, float pxRange, float midValue) {
    msdfVec2 scale((float) sdf.width/output.width, (float) sdf.height/output.height);
    pxRange *= (float) (output.width+output.height)/(sdf.width+sdf.height);
    for (int y = 0; y < output.height; ++y)
      for (int x = 0; x < output.width; ++x) {
  ...
  void renderSDF(const BitmapRef<float, 3> &output, const BitmapConstRef<float, 3> &sdf, float pxRange, float midValue) {
    msdfVec2 scale((float) sdf.width/output.width, (float) sdf.height/output.height);
    pxRange *= (float) (output.width+output.height)/(sdf.width+sdf.height);
    for (int y = 0; y < output.height; ++y)
      for (int x = 0; x < output.width; ++x) {
  ...
  void renderSDF(const BitmapRef<float, 1> &output, const BitmapConstRef<float, 4> &sdf, float pxRange, float midValue) {
    msdfVec2 scale((float) sdf.width/output.width, (float) sdf.height/output.height);
    pxRange *= (float) (output.width+output.height)/(sdf.width+sdf.height);
    for (int y = 0; y < output.height; ++y)
      for (int x = 0; x < output.width; ++x) {
  ...
  void renderSDF(const BitmapRef<float, 4> &output, const BitmapConstRef<float, 4> &sdf, float pxRange, float midValue) {
    msdfVec2 scale((float) sdf.width/output.width, (float) sdf.height/output.height);
    pxRange *= (float) (output.width+output.height)/(sdf.width+sdf.height);
    for (int y = 0; y < output.height; ++y)
      for (int x = 0; x < output.width; ++x) {
