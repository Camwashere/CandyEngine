...
namespace msdfgen {
  ...
  static bool isCorner(const msdfVec2 &aDir, const msdfVec2 &bDir, float crossThreshold) {
    return dotProduct(aDir, bDir) <= 0 || fabs(crossProduct(aDir, bDir)) > crossThreshold;
  }
  
  static float estimateEdgeLength(const EdgeSegment *edge) {
    float len = 0;
    Point2 prev = edge->point(0);
    for (int i = 1; i <= MSDFGEN_EDGE_LENGTH_PRECISION; ++i) {
      Point2 cur = edge->point(1.f/MSDFGEN_EDGE_LENGTH_PRECISION*i);
      len += (cur-prev).length();
      prev = cur;
    }
    return len;
  ...
  void edgeColoringSimple(Shape &shape, float angleThreshold, unsigned long long seed) {
    float crossThreshold = std::sin(angleThreshold);
    std::vector<int> corners;
    for (std::vector<Contour>::iterator contour = shape.contours.begin(); contour != shape.contours.end(); ++contour) {
      // Identify corners
      corners.clear();
      if (!contour->edges.empty()) {
        msdfVec2 prevDirection = contour->edges.back()->direction(1);
        int index = 0;
  ...
  struct EdgeColoringInkTrapCorner {
    int index;
    float prevEdgeLengthEstimate;
    bool minor;
    EdgeColor color;
  };
  
  void edgeColoringInkTrap(Shape &shape, float angleThreshold, unsigned long long seed) {
...
#define EDGE_DISTANCE_PRECISION 16
  ...
  static float edgeToEdgeDistance(const EdgeSegment &a, const EdgeSegment &b, int precision) {
    if (a.point(0) == b.point(0) || a.point(0) == b.point(1) || a.point(1) == b.point(0) || a.point(1) == b.point(1))
      return 0;
    float iFac = 1.f/precision;
    float minDistance = (b.point(0)-a.point(0)).length();
    for (int i = 0; i <= precision; ++i) {
      float t = iFac*i;
      float d = std::fabs(a.signedDistance(b.point(t), t).distance);
      minDistance = min(minDistance, d);
    }
    for (int i = 0; i <= precision; ++i) {
      float t = iFac*i;
      float d = fabs(b.signedDistance(a.point(t), t).distance);
      minDistance = min(minDistance, d);
    }
    return minDistance;
  }
  
  static float splineToSplineDistance(EdgeSegment * const *edgeSegments, int aStart, int aEnd, int bStart, int bEnd, int precision) {
    float minDistance = DBL_MAX;
    for (int ai = aStart; ai < aEnd; ++ai)
      for (int bi = bStart; bi < bEnd && minDistance; ++bi) {
        float d = edgeToEdgeDistance(*edgeSegments[ai], *edgeSegments[bi], precision);
        minDistance = min(minDistance, d);
      }
    return minDistance;
  }
  ...
  static int cmpDoublePtr(const void *a, const void *b) {
    return sign(**reinterpret_cast<const float * const *>(a)-**reinterpret_cast<const float * const *>(b));
  }
  
  void edgeColoringByDistance(Shape &shape, float angleThreshold, unsigned long long seed) {
    ...
    std::vector<float> distanceMatrixStorage(splineCount*splineCount);
    std::vector<float *> distanceMatrix(splineCount);
    for (int i = 0; i < splineCount; ++i)
      distanceMatrix[i] = &distanceMatrixStorage[i*splineCount];
    const float *distanceMatrixBase = &distanceMatrixStorage[0];
    
    for (int i = 0; i < splineCount; ++i) {
      distanceMatrix[i][i] = -1;
      for (int j = i+1; j < splineCount; ++j) {
        float dist = splineToSplineDistance(&edgeSegments[0], splineStarts[i], splineStarts[i+1], splineStarts[j], splineStarts[j+1], EDGE_DISTANCE_PRECISION);
        distanceMatrix[i][j] = dist;
        distanceMatrix[j][i] = dist;
      }
    }
    
    std::vector<const float *> graphEdgeDistances;
    ...
    if (!graphEdgeDistances.empty())
      qsort(&graphEdgeDistances[0], graphEdgeDistances.size(), sizeof(const float *), &cmpDoublePtr);
