...
namespace msdfgen {
  ...
  float Scanline::overlap(const Scanline &a, const Scanline &b, float xFrom, float xTo, FillRule fillRule) {
    float total = 0;
    bool aInside = false, bInside = false;
    int ai = 0, bi = 0;
    float ax = !a.intersections.empty() ? a.intersections[ai].x : xTo;
    float bx = !b.intersections.empty() ? b.intersections[bi].x : xTo;
    while (ax < xFrom || bx < xFrom) {
      float xNext = min(ax, bx);
  ...
  int Scanline::moveTo(float x) const {
    if (intersections.empty())
      return -1;
    int index = lastIndex;
  ...
  int Scanline::countIntersections(float x) const {
    return moveTo(x)+1;
  }
  
  int Scanline::sumIntersections(float x) const {
    int index = moveTo(x);
    if (index >= 0)
  ...
  bool Scanline::filled(float x, FillRule fillRule) const {
