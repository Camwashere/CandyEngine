...
bool exportJSON(const FontGeometry *fonts, int fontCount, float fontSize, float pxRange, int atlasWidth, int atlasHeight, ImageType imageType, YDirection yDirection, const char *filename, bool kerning) {
    ...
    for (int i = 0; i < fontCount; ++i) {
        ...
        // Font metrics
        fputs("\"metrics\":{", f); {
            float yFactor = yDirection == YDirection::TOP_DOWN ? -1 : 1;
        ...
        if (kerning) {
            ...
            switch (font.getPreferredIdentifierType()) {
                case GlyphIdentifierType::GLYPH_INDEX:
                    for (const std::pair<std::pair<int, int>, float> &kernPair : font.getKerning()) {
                        fputs(firstPair ? "{" : ",{", f);
                ...
                case GlyphIdentifierType::UNICODE_CODEPOINT:
                    for (const std::pair<std::pair<int, int>, float> &kernPair : font.getKerning()) {
                        const GlyphGeometry *glyph1 = font.getGlyph(GlyphIndex(kernPair.first.first));
                        const GlyphGeometry *glyph2 = font.getGlyph(GlyphIndex(kernPair.first.second));
