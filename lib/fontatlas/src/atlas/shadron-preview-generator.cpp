...
bool generateShadronPreview(const FontGeometry *fonts, int fontCount, ImageType atlasType, int atlasWidth, int atlasHeight, float pxRange, const unicode_t *text, const char *imageFilename, bool fullRange, const char *outputFilename) {
    if (fontCount <= 0)
        return false;
    float texelWidth = 1.f/atlasWidth;
    float texelHeight = 1.f/atlasHeight;
    ...
    {
        FontMetrics fontMetrics = fonts->getMetrics();
        ...
        float fsScale = 1/(fontMetrics.ascenderY-fontMetrics.descenderY);
        fputs("vertex_list GlyphVertex textQuadVertices = {\n", file);
        float x = 0, y = -fsScale*fontMetrics.ascenderY;
        float textWidth = 0;
