...
int FontGeometry::loadGlyphset(FontHandle *font, float fontScale, const Charset &glyphset, bool preprocessGeometry, bool enableKerning) {
    ...
    for (unicode_t index : glyphset) {
        GlyphGeometry glyph;
        if (glyph.load(font, geometryScale, GlyphIndex(index), preprocessGeometry)) {
...
int FontGeometry::loadCharset(FontHandle *font, float fontScale, const Charset &charset, bool preprocessGeometry, bool enableKerning) {
    if (!(glyphs->size() == rangeEnd && loadMetrics(font, fontScale)))
        return -1;
...
bool FontGeometry::loadMetrics(FontHandle *font, float fontScale) {
    if (!FontAtlasLoader::GetFontMetrics(metrics, font))
        return false;
    if (metrics.emSize <= 0)
...
int FontGeometry::loadKerning(FontHandle *font) {
    int loaded = 0;
    for (size_t i = rangeStart; i < rangeEnd; ++i)
        for (size_t j = rangeStart; j < rangeEnd; ++j) {
            float advance;
            if (FontAtlasLoader::GetKerning(advance, font, (*glyphs)[i].getGlyphIndex(), (*glyphs)[j].getGlyphIndex()) && advance) {
...
float FontGeometry::getGeometryScale() const {
    return geometryScale;
}

const FontMetrics & FontGeometry::getMetrics() const {
    return metrics;
}
...
const GlyphGeometry * FontGeometry::getGlyph(GlyphIndex index) const {
...
bool FontGeometry::getAdvance(float &advance, GlyphIndex index1, GlyphIndex index2) const {
    const GlyphGeometry *glyph1 = getGlyph(index1);
    if (!glyph1)
        return false;
    advance = glyph1->getAdvance();
    std::map<std::pair<int, int>, float>::const_iterator it = kerning.find(std::make_pair<int, int>(index1.GetIndex(), index2.GetIndex()));
    if (it != kerning.end())
        advance += it->second;
    return true;
}

bool FontGeometry::getAdvance(float &advance, unicode_t codepoint1, unicode_t codepoint2) const {
    const GlyphGeometry *glyph1, *glyph2;
    if (!((glyph1 = getGlyph(codepoint1)) && (glyph2 = getGlyph(codepoint2))))
        return false;
    advance = glyph1->getAdvance();
    std::map<std::pair<int, int>, float>::const_iterator it = kerning.find(std::make_pair<int, int>(glyph1->getIndex(), glyph2->getIndex()));
    if (it != kerning.end())
        advance += it->second;
    return true;
}

const std::map<std::pair<int, int>, float> & FontGeometry::getKerning() const {
    return kerning;
}
