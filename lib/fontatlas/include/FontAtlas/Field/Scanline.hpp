...
/// Represents a horizontal scanline intersecting a shape.
  ...
  public:
    ...
    static float overlap(const Scanline &a, const Scanline &b, float xFrom, float xTo, FillRule fillRule);
    ...
    int countIntersections(float x) const;
    /// Returns the total sign of intersections left of x.
    int sumIntersections(float x) const;
    /// Decides whether the scanline is filled at x based on fill rule.
    bool filled(float x, FillRule fillRule) const;
