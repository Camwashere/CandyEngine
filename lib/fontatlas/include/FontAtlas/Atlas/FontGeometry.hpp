...
public:
    ...
    int loadGlyphset(FontHandle *font, float fontScale, const Charset &glyphset, bool preprocessGeometry = true, bool enableKerning = true);
    /// Loads all glyphs in a charset (Charset elements are Unicode codepoints), returns the number of successfully loaded glyphs
    int loadCharset(FontHandle *font, float fontScale, const Charset &charset, bool preprocessGeometry = true, bool enableKerning = true);

    /// Only loads font metrics and geometry scale from font
    bool loadMetrics(FontHandle *font, float fontScale);
    /// Adds a loaded glyph
    ...
    int loadKerning(FontHandle *font);
    ...
    const GlyphGeometry * getGlyph(GlyphIndex index) const;
    ...
    bool getAdvance(float &advance, GlyphIndex index1, GlyphIndex index2) const;
    bool getAdvance(float &advance, unicode_t codepoint1, unicode_t codepoint2) const;
    /// Returns the complete mapping of kerning pairs (by glyph indices) and their respective advance values
    const std::map<std::pair<int, int>, float> & getKerning() const;
